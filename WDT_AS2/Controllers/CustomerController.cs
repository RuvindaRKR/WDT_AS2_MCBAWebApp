...
namespace WDT_AS2.Controllers
{
    ...
    public class CustomerController : Controller
    {
        ...
        {
            //  retrieve account with specified id
            var account = await _context.Accounts.FindAsync(id);

            //  run validation checks for amount and go back if invalid
            if(amount <= 0)
        ...
        {
            // retrieve data from database
            var billpays = await _context.BillPays.FindAsync(id);
